...
void flood_fill(int n, int m){
    if(n == -1 || m == -1 || n == N || m == M || castle[n][m].visited) return;
    else{
...
int main(){
    ...
    castle.resize(N);
    for(int i = 0; i < N; i++) castle[i].resize(M);
 
    for(int i = 0; i < N; i++){ //Input and corridor assignment
        for(int j = 0; j < M; j++){
            int num;
            in >> num;
    ...
    for(int i = 0; i < N; i++){
        for(int j = 0; j < M; j++){
            if(j != M - 1 && castle[i][j].walls[1] && castle[i][j].room != castle[i][j + 1].room){
